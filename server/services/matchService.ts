import { formatBeijingDateTime, getBeijingDayRangeUtc } from "../../shared/date.js";
import { GameFilter, GameType, Match, MatchesResponse } from "../../shared/match.js";
...
function sortByBeginAt(matches: Match[]): Match[] {
  return [...matches].sort((left, right) => {
    return new Date(left.beginAt).getTime() - new Date(right.beginAt).getTime();
  });
}
...
export async function getMatches({ date, game, refresh = false }: GetMatchesArgs): Promise<MatchesResponse> {
  const cacheKey = buildCacheKey(date, game);

  const freshCachedResponse = refresh ? null : getFresh<MatchesResponse>(cacheKey);
  ...
  try {
    const matchGroups: PandaScoreMatch[][] =
      game === "all"
        ? await Promise.all([
            fetchPandaScoreMatches("cs2", date),
            fetchPandaScoreMatches("valorant", date),
            fetchPandaScoreMatches("lol", date)
          ])
        : [await fetchPandaScoreMatches(game, date)];

    const matches = sortByBeginAt(
        matchGroups.flatMap((group: PandaScoreMatch[], index: number) => {
        const groupGame: GameType = game === "all" ? (["cs2", "valorant", "lol"][index] as GameType) : game;

          return group.map((match: PandaScoreMatch) => mapPandaScoreMatch(match, groupGame));
      })
    );
